...
- Full outer join between LEFT and RIGHT (keeps all rows).
- Optional high-confidence fuzzy join to merge near-miss names.
- Optional EXTRA workbook with multiple sheets; columns are suffixed with the sheet name.
- By default, EXTRA workbook is left-joined (enrich-only) to avoid flooding with 'extra_only' rows.

OUTPUT sheets:
- merged
- stats (matched/left_only/right_only/extra_only if any)
- match_methods (exact vs fuzzy counts)
- coverage (how many rows have data for each extra sheet)
- unmatched_from_left
...
DEFAULTS = {
    ...
    # How to join extra sheets to the combined table:
    #   "left"  -> enrich only existing players (default; avoids 'extra_only' rows)
    #   "outer" -> include players that appear only in extra sheets
    "extra_join": "left",
...
def load_excel_with_player_column(path: Path, sheet: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if player_col is not None:
            ...
            # Ensure we don't carry a stray 'Nome' column anymore
    ...
    df["__giocatore_raw"] = df["giocatore"].astype(str)
    df["__giocatore_norm"] = df["__giocatore_raw"].map(normalize_name)
    # Remove any lingering 'Nome'/'nome' columns to avoid unsuffixed duplicates
